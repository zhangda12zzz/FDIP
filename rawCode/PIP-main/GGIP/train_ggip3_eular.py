...
from GGIP.ggip_net import AGGRU_1, AGGRU_2, AGGRU_3, euler2mat
...
if __name__ == '__main__':
    ...
    rnn3 = AGGRU_3(24*3+16*12, 256, 15*3).to(device)
    ...
    writer = SummaryWriter('GGIP/log/ggip3_eular_matRefine')
    
    # 预训练的设置
    pretrain_path = 'GGIP/checkpoints/ggip3_eular/epoch_400.pkl'
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            pose_mat = data[5].to(device).float()               # [batch_size, max_seq, 15, 3]
            pose = data[7].to(device).float()               # [batch_size, max_seq, 15, 3]
            n,t,_,_ = acc.shape
            ...
            target = pose.view(-1, pose.shape[1], 15*3)
            target_mat = pose_mat.view(-1, pose.shape[1], 15*9)
            
            rnn3.train()
            logits = rnn3(input)
            logits_mat = euler2mat(logits)
            optimizer.zero_grad()
            
            # 改了mseLoss为mean之后的计算（纯粹为了比较）
            loss = criterion(logits, target)
            loss_mat = criterion(logits_mat, target_mat)
            
            if log_on:
                writer.add_scalar('mse_step/train', loss, epoch_step)
                writer.add_scalar('mse_mat_step/train', loss_mat, epoch_step)
            train_loss_list.append(loss)
            train_loss_mat_list.append(loss_mat)
                
            loss_all = 1 * loss + 1 * loss_mat
        ...
        avg_train_mat_loss = sum(train_loss_mat_list) / len(train_loss_mat_list)
        if log_on:
            writer.add_scalar('mse/train', avg_train_loss, epoch)
            writer.add_scalar('mse_mat/train', avg_train_mat_loss, epoch)
        ...
        with torch.no_grad():
            ...
            for batch_idx_val, data_val in enumerate(val_loader):
                ...
                pose_mat_val = data_val[5].to(device).float()               # [batch_size, max_seq, 15, 3]
                pose_val = data_val[7].to(device).float()               # [batch_size, max_seq, 15, 3]
                ...
                target_val = pose_val.view(-1, pose_val.shape[1], 15*3)
                target_mat_val = pose_mat_val.view(-1, pose_mat_val.shape[1], 15*9)
                
                logits_val = rnn3(input_val)
                logits_mat_val = euler2mat(logits_val)

                # 损失计算
                loss_val = criterion(logits_val, target_val)
                loss_mat_val = criterion(logits_mat_val, target_mat_val)
                if log_on:
                    writer.add_scalar('mse_step/val', loss_val, epoch_step)
                    writer.add_scalar('mse_mat_step/val', loss_mat_val, epoch_step)
                    
                epoch_step += 1
                val_loss_list.append(loss_val)
                val_loss_mat_list.append(loss_mat_val)
                
            avg_val_loss = sum(val_loss_list) / len(val_loss_list)
            avg_val_mat_loss = sum(val_loss_mat_list) / len(val_loss_mat_list)
            if log_on:
                writer.add_scalar('mse/val', avg_val_loss, epoch)
                writer.add_scalar('mse_mat/val', avg_val_mat_loss, epoch)
        ...
        if (epoch+pretrain_epoch) % checkpoint_interval == 0:
            checkpoint = {"model_state_dict": rnn3.state_dict(),
                    "optimizer_state_dict": optimizer.state_dict(),
                    "epoch": epoch+pretrain_epoch}
            path_checkpoint = "GGIP/checkpoints/ggip3_eular_matRefine/epoch_{}.pkl".format(epoch+pretrain_epoch)
            torch.save(checkpoint, path_checkpoint)
