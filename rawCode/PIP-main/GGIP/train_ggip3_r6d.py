...
if __name__ == '__main__':
    ...
    writer = SummaryWriter('GGIP/log/ggip3')
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            input = torch.concat((p_all, full_acc, full_ori), dim=-1)
            
            target = pose.view(-1, pose.shape[1], 90)
        ...
        with torch.no_grad():
            ...
            for batch_idx_val, data_val in enumerate(val_loader):
                ...
                input_val = torch.concat((p_all_val, full_acc_val, full_ori_val), dim=-1)
                
                target_val = pose_val.view(-1, pose_val.shape[1], 90)
        ...
        if (epoch+pretrain_epoch) % checkpoint_interval == 0:
            checkpoint = {"model_state_dict": rnn3.state_dict(),
                    "optimizer_state_dict": optimizer.state_dict(),
                    "epoch": epoch+pretrain_epoch}
            path_checkpoint = "GGIP/checkpoints/ggip3/epoch_{}.pkl".format(epoch+pretrain_epoch)
            torch.save(checkpoint, path_checkpoint)
