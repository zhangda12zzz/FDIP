...
def reduced_local_to_global(batch, seq, glb_reduced_pose, r6d=False, root_rotation=None, shape=None):
    ...
    pose = global_full_pose.view(-1,24,3,3).contiguous() #[n,t,24,3,3]->[nt,24,3,3]
...
if __name__ == '__main__':
    ...
    writer = SummaryWriter('GGIP/log/ggip3_r6d_lossOpt')
    
    # 预训练的设置
    pretrain_path = 'GGIP/checkpoints/ggip3_r6d_lossopt/epoch_320.pkl'
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            pose = data[6].float()               # [batch_size, max_seq, 15, 6]
            pose_glb = data[5].float()
            beta = data[7]
            ...
            target_r6d = art.math.rotation_matrix_to_r6d(target).view(-1,15,6).to(device)    #[ntv,6]
            ...
            loss_r6d = criterion(logits, target_r6d.view(n,t,90))
            loss_all = loss / 100.0 + loss_r6d
            
            if log_on:
                writer.add_scalar('mse_pos_step/train', loss, epoch_step)
            train_loss_list.append(loss)
            train_loss_r6d_list.append(loss_r6d)
        ...
        with torch.no_grad():
            ...
            for batch_idx_val, data_val in enumerate(val_loader):
                ...
                pose_val = data_val[6].float()               # [batch_size, max_seq, 15, 6]
                beta_val = data_val[7]
                ...
                _, target_pos_val = reduced_local_to_global(n,t,target_val, shape=beta_val)
                
                logits_val = rnn3(input_val)
                _, logits_pos_val = reduced_local_to_global(n,t,logits_val, r6d=True, shape=beta_val)

                # 损失计算
                logits_pos_used_val = logits_pos_val[:,conf.joint_set.reduced_pos] * 100
                target_pos_used_val = target_pos_val[:,conf.joint_set.reduced_pos].to(device) * 100
                loss_val = criterion(logits_pos_used_val, target_pos_used_val)
                if log_on:
                    writer.add_scalar('mse_pos_step/val', loss_val, epoch_step)
            ...
            if log_on:
                writer.add_scalar('mse_pos/val', avg_val_loss, epoch)
        ...
        if (epoch+pretrain_epoch) % checkpoint_interval == 0:
            checkpoint = {"model_state_dict": rnn3.state_dict(),
                    "optimizer_state_dict": optimizer.state_dict(),
                    "epoch": epoch+pretrain_epoch}
            path_checkpoint = "GGIP/checkpoints/ggip3_r6d_lossopt/epoch_{}.pkl".format(epoch+pretrain_epoch)
            torch.save(checkpoint, path_checkpoint)
