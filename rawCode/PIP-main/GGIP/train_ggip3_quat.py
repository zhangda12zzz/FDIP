...
if __name__ == '__main__':
    ...
    rnn3 = AGGRU_3(24*3+16*12, 256, 15*4, is_quat=True).to(device)
    ...
    writer = SummaryWriter('GGIP/log/ggip3_quat')
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            pose = data[8].to(device).float()               # [batch_size, max_seq, 15, 4]
            n,t,_,_ = acc.shape
            ...
            target = pose.view(-1, pose.shape[1], 15*4)
        ...
        with torch.no_grad():
            ...
            for batch_idx_val, data_val in enumerate(val_loader):
                ...
                pose_val = data_val[8].to(device).float()               # [batch_size, max_seq, 15, 4]
                ...
                target_val = pose_val.view(-1, pose_val.shape[1], 15*4)
        ...
        if (epoch+pretrain_epoch) % checkpoint_interval == 0:
            checkpoint = {"model_state_dict": rnn3.state_dict(),
                    "optimizer_state_dict": optimizer.state_dict(),
                    "epoch": epoch+pretrain_epoch}
            path_checkpoint = "GGIP/checkpoints/ggip3_quat/epoch_{}.pkl".format(epoch+pretrain_epoch)
            torch.save(checkpoint, path_checkpoint)
