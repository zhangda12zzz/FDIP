...
if __name__ == '__main__':
    ...
    writer = SummaryWriter('GGIP/log/ggip3_axis')
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            pose = data[6].to(device).float()               # [batch_size, max_seq, 15, 3]
            n,t,_,_ = acc.shape
        ...
        with torch.no_grad():
            ...
            for batch_idx_val, data_val in enumerate(val_loader):
                ...
                pose_val = data_val[6].to(device).float()               # [batch_size, max_seq, 15, 3]
        ...
        if (epoch+pretrain_epoch) % checkpoint_interval == 0:
            checkpoint = {"model_state_dict": rnn3.state_dict(),
                    "optimizer_state_dict": optimizer.state_dict(),
                    "epoch": epoch+pretrain_epoch}
            path_checkpoint = "GGIP/checkpoints/ggip3_axis/epoch_{}.pkl".format(epoch+pretrain_epoch)
            torch.save(checkpoint, path_checkpoint)
