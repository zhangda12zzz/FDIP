...
def run_pipeline(net, data_dir, sequence_ids=None):
    ...
    os.makedirs(output_dir, exist_ok=True)  # 确定输出目录
    ...
    for i in tqdm.tqdm(sequence_ids):
        ...
        # 将 net.predict(）的结果按照sequence_ids的序列存到目录文件里
        # 输入参数为：加速度、速度、初始姿态 
        
        # TransPoseNet:
        # x = normalize_and_concat(accs[i] / 30, rots[i]).to(device)
        # torch.save(net.forward_offline(x), os.path.join(output_dir, '%d.pt' % i))
...
def evaluate(net, data_dir, sequence_ids=None, flush_cache=False, pose_evaluator=ReducedPoseEvaluator(),
    ...
    missing_ids = [i for i in sequence_ids if not os.path.exists(os.path.join(result_dir, '%d.pt' % i))]    # 还没跑出结果的序列
    cached_ids = [i for i in sequence_ids if os.path.exists(os.path.join(result_dir, '%d.pt' % i))]         # 之前已经跑出结果的序列
    print('Cached ids: %s\nMissing ids: %s' % (cached_ids, missing_ids))
    if len(missing_ids) > 0:    # 如果有没跑完的结果
        run_pipeline(net, data_dir, missing_ids)    # 开始训练！输入（网络net，数据文件夹data_dir，还没跑完的序列missing_ids）

    # 下面就是将预测值和真值比较、并计算loss
    pose_errors = []
