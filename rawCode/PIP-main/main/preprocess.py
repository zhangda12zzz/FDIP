...
def process_amass():
    ...
    b = 0
    out_pose, out_shape, out_tran, out_joint, out_vrot, out_vacc, out_grot, out_jvel = [], [], [], [], [], [], [], []
    out_grot_axis, out_grot_euler,out_grot_quat = [],[],[]
    for i, l in tqdm(list(enumerate(length))):
        ...
        # out_grot.append(grot[:, :24].contiguous().clone())  # N, 24, 3, 3
        # out_jvel.append(_syn_vel(joint[:, :24].contiguous().clone(), grot[:, 0])) # N, 24, 3
        
        out_grot_raw = torch.cat((grot[:, :1], grot[:, :1].transpose(2, 3).matmul(grot[:, 1:24])), dim=1)
        # out_grot_axis.append(art.math.rotation_matrix_to_axis_angle(out_grot_raw).contiguous().clone())
        # out_grot_euler.append(art.math.rotation_matrix_to_euler_angle(out_grot_raw).contiguous().clone())
        out_grot_quat.append(art.math.rotation_matrix_to_quat(out_grot_raw).contiguous().clone())
        
        b += l

    print('Saving')
    os.makedirs(paths.amass_dir, exist_ok=True)
    # torch.save(out_pose, os.path.join(paths.amass_dir, 'train/pose.pt'))
    # torch.save(out_shape, os.path.join(paths.amass_dir, 'train/shape.pt'))
    # torch.save(out_tran, os.path.join(paths.amass_dir, 'train/tran.pt'))
    # torch.save(out_joint, os.path.join(paths.amass_dir, 'train/joint.pt'))
    # torch.save(out_vrot, os.path.join(paths.amass_dir, 'train/vrot.pt'))
    # torch.save(out_vacc, os.path.join(paths.amass_dir, 'train/vacc.pt'))
    # torch.save(out_grot, os.path.join(paths.amass_dir, 'train/grot.pt'))
    # torch.save(out_jvel, os.path.join(paths.amass_dir, 'train/jvel.pt'))
    # torch.save(out_grot_axis, os.path.join(paths.amass_dir, 'train/grot_axis.pt'))
    # torch.save(out_grot_euler, os.path.join(paths.amass_dir, 'train/grot_euler.pt'))
    torch.save(out_grot_quat, os.path.join(paths.amass_dir, 'train/grot_quat.pt'))
...
def process_dipimu_train():
    ...
    out_pose, out_tran, out_joint, out_vrot, out_vacc, out_grot, out_jvel, out_shape = [], [], [], [], [], [], [], []
    out_grot_axis, out_grot_euler, out_grot_quat = [],[],[]
    for i in tqdm(range(length)):
        p = art.math.axis_angle_to_rotation_matrix(poses[i]).view(-1, 24, 3, 3)
        grot, joint = body_model.forward_kinematics(p)
        out_shape.append(torch.zeros(p.shape[0], 10))
    ...
    print('Saving')
    os.makedirs(paths.dipimu_dir, exist_ok=True)
    # torch.save(out_pose, os.path.join(paths.dipimu_dir, 'train/pose.pt'))
    # torch.save(out_tran, os.path.join(paths.dipimu_dir, 'train/tran.pt'))
    # torch.save(out_joint, os.path.join(paths.dipimu_dir, 'train/joint.pt'))
    # torch.save(out_vrot, os.path.join(paths.dipimu_dir, 'train/vrot.pt'))
    # torch.save(out_vacc, os.path.join(paths.dipimu_dir, 'train/vacc.pt'))
    # torch.save(out_grot, os.path.join(paths.dipimu_dir, 'train/grot.pt'))
    # torch.save(out_jvel, os.path.join(paths.dipimu_dir, 'train/jvel.pt'))
    # torch.save(out_grot_axis, os.path.join(paths.dipimu_dir, 'train/grot_axis.pt'))
    # torch.save(out_grot_euler, os.path.join(paths.dipimu_dir, 'train/grot_euler.pt'))
    # torch.save(out_grot_quat, os.path.join(paths.dipimu_dir, 'train/grot_quat.pt'))
    torch.save(out_shape, os.path.join(paths.dipimu_dir, 'train/shape.pt'))
    print('Synthetic DIP dataset is saved at', paths.dipimu_dir)
...
def process_totalcapture():
    ...
    out_pose, out_tran, out_joint, out_vrot, out_vacc, out_grot, out_jvel = [], [], [], [], [], [], []
    for i in tqdm(range(length)):
        p = art.math.axis_angle_to_rotation_matrix(poses[i]).view(-1, 24, 3, 3)
        grot, joint = body_model.forward_kinematics(p, tran=trans[i])
        out_pose.append(poses[i].clone())  # N, 24, 3
        out_tran.append(trans[i].clone())  # N, 3
        out_joint.append(joint[:, :24].contiguous().clone())  # N, 24, 3
        out_vacc.append(accs[i].clone())  # N, 6, 3
    ...
    print('Saving')
    os.makedirs(paths.totalcapture_dir, exist_ok=True)
    torch.save(out_pose, os.path.join(paths.totalcapture_dir, 'train/pose.pt'))
    torch.save(out_tran, os.path.join(paths.totalcapture_dir, 'train/tran.pt'))
    torch.save(out_joint, os.path.join(paths.totalcapture_dir, 'train/joint.pt'))
    torch.save(out_vrot, os.path.join(paths.totalcapture_dir, 'train/vrot.pt'))
    torch.save(out_vacc, os.path.join(paths.totalcapture_dir, 'train/vacc.pt'))
    torch.save(out_grot, os.path.join(paths.totalcapture_dir, 'train/grot.pt'))
    torch.save(out_jvel, os.path.join(paths.totalcapture_dir, 'train/jvel.pt'))
