...
class PhysicsOptimizer:
    ...
    def optimize_frame(self, pose, jvel, contact, acc):
        ...
        # contact: [2]
        # acc: [6, 3]
        q_ref = smpl_to_rbdl(pose, torch.zeros(3))[0]   #[75]？(3 root position + 72 joint rotation)
        v_ref = jvel.numpy()                            # 关节速度[24，3]
        c_ref = contact.sigmoid().numpy()               # 脚地接触[2]
        a_ref = acc.numpy()                             # 加速度[6,3]
        q = self.q
        qdot = self.qdot

        if q is None:   # 初始没有的话就设置第一帧为初始
            self.q = q_ref
        ...
        for joint_name in self.test_contact_joints:
            ...
            pos = self.model.calc_body_position(q, joint_id)    # 计算可能的接触点的位置
            # 左脚 / 右脚 / 其他点位置在水平线以下，视为接触关节
            if joint_id == Body.LFOOT and c_ref[0] > 0.5 and pos[1] <= self.params['floor_y'] + 0.03 or \
               ...
               pos[1] <= self.params['floor_y']:
                ...
                for ps in self.support_polygon + pos:   # 计算接触关节的接触点
        ...
        Gs1, hs1, Gs2, hs2, Gs3, hs3 = [np.zeros((0, self.model.qdot_size))], [np.empty(0)], [np.empty((0, nc * 3))], \
                                       [np.empty(0)], [np.zeros((0, self.model.qdot_size))], [np.empty(0)]
        A_, b_ = None, None     # 以上这些都是初始化，要解二次规划问题
