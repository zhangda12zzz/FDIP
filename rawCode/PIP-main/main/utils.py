...
def smpl_to_rbdl(poses, trans):
    ...
    euler_poses = rotation_matrix_to_euler_angle_np(poses[:, 1:], 'XYZ').reshape(-1, 69)    # 非根节点的旋转矩阵全部变成XYZ顺序内旋的欧拉角
    euler_glbrots = rotation_matrix_to_euler_angle_np(poses[:, :1], 'xyz').reshape(-1, 3)   # 根节点的旋转矩阵变成xyz顺序外旋的欧拉角
    euler_glbrots = euler_convert_np(euler_glbrots[:, [2, 1, 0]], 'xyz', 'zyx')             # 根节点的旋转矩阵变成zyx顺序外旋的欧拉角
    qs = np.concatenate((trans, euler_glbrots, euler_poses[:, _smpl_to_rbdl]), axis=1)
    qs[:, 3:] = normalize_angle(qs[:, 3:])  # 将幅度标准化为[-pi, pi)
    return qs
...
def normalize_and_concat(glb_acc, glb_rot):
    ...
    acc = torch.cat((glb_acc[:, :5] - glb_acc[:, 5:], glb_acc[:, 5:]), dim=1).bmm(glb_rot[:, -1]) #/ conf.acc_scale  # PIP原代码里是没有这个conf.acc_scale的，tp才有
