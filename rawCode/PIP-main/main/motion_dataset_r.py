...
from .bvh_parser_r import BvhData
...
class MotionDataset(Dataset):
    def __init__(self, args, mode):
        ...
        for character_name in [self.character_a_name, self.character_b_name]:
            std_bvh_data = BvhData(character_name, motion_file_name=character_name) # T-pose获取数据
    ...
    def __getitem__(self, idx):
        ...
        for idx,  character_name in enumerate([self.character_a_name, self.character_b_name]):
            ...
            # 但是在使用RNN的基础上，我想修改一下这个部分，我想确认它剪切的大小（即self.joint_nums（即args.window_size）的大小）
            # 这里返回的原本是 [B, 7, simple_joint_num, args.window_size]
            # 我想确认的是:     B = 1 ?是什么情况
            #                 非训练状态、不裁剪、会是什么情况？
            #                 args.window_size可以是200吗？
            if self.args.is_train:
                final_output = self._slice_to_equal_frame_len(final_output)
                
            # final_output处理成六位数据（更改joint）
            # leaf_joints = []    # TODO:这里记得加上
            # final_output = final_output[:,:, leaf_joints, :]
