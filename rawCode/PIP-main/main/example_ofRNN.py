...
from net import PIP, TransPoseNet, PIP_rebuild
from data.process_final import makeSingleBvh
...
def run_pipeline(net, data_dir, pose_t_all, sequence_ids=None):
    ...
    for i in tqdm.tqdm(sequence_ids):
        # a_pose = net.predictPose(accs[i], rots[i], init_poses[i])   # SIP: 25.71 & 24.38
        a_pose = net.predictPose(accs[i], rots[i])   # SIP: 15.xx
        # a_pose = net.predictPose(accs[i]/30.0, rots[i])
        # makeSingleBvh("data/dataset_bvh/DIP_IMU_pred/", str(i), a_pose)
        pose_t = pose_t_all[i]
        # poses[i] == pose_t  YES!!!
