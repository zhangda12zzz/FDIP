...
if __name__ == '__main__':
    ...
    optimizer = optim.Adam(rnn4.parameters(), lr=learning_rate)

    val_best_loss = 10000.0
    test_loss = 0
    
    # 预训练的设置
    pretrain_path = 'checkpoints/trial1210/standard_biRNN23/rnn4/rnn4__checkpoint_best_61_epoch_2.5892724990844727.pkl'
    pretrain_data = torch.load(pretrain_path)
    rnn4.load_state_dict(pretrain_data['model_state_dict'])
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            if test_loss < val_best_loss:
                ...
                print('\nval loss reset')
                checkpoint = {"model_state_dict": rnn4.state_dict(),
                        "optimizer_state_dict": optimizer.state_dict(),
                        "epoch": epoch+pretrain_epoch}
                if epoch > 0:
                    path_checkpoint = "./checkpoints/trial1210/standard_biRNN23/rnn4/rnn4__checkpoint_best_{}_epoch_{}.pkl".format(epoch+pretrain_epoch, test_loss)
        ...
        if epoch % checkpoint_interval == 0:
            checkpoint = {"model_state_dict": rnn4.state_dict(),
                    "optimizer_state_dict": optimizer.state_dict(),
                    "epoch": epoch+pretrain_epoch}
            path_checkpoint = "./checkpoints/trial1210/standard_biRNN23/rnn4/rnn4__checkpoint_{}_epoch_{}.pkl".format(epoch+pretrain_epoch, test_loss)
            torch.save(checkpoint, path_checkpoint)
            
    checkpoint = {"model_state_dict": rnn4.state_dict(),
                      "optimizer_state_dict": optimizer.state_dict(),
                      "epoch": epochs+pretrain_epoch-1}
    path_checkpoint = "./checkpoints/trial1210/standard_biRNN23/rnn4/rnn4__checkpoint_final_{}_epoch_{}.pkl".format(epochs+pretrain_epoch-1, test_loss)
    torch.save(checkpoint, path_checkpoint)