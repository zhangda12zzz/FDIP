...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        for batch_idx, data in enumerate(train_loader):
            ...
            # 损失计算
            loss_mat = criterion(logits[2], target[0]).to(device)
            seq_len = acc.shape[1]
        ...
        with torch.no_grad():
            for batch_idx_val, data_val in enumerate(val_loader):
                ...
                logits_val = net(input_val)

                # 损失计算
                loss_mat_val = criterion(logits_val[2], target_val[0]).to(device)
            ...
            if test_loss < val_best_loss:
                ...
                if epoch > 0:
                    path_checkpoint = "./checkpoints/trial1207/all_modify/net__checkpoint_best_{}_epoch_{}.pkl".format(epoch+pretrain_epoch, test_loss)
        ...
        if epoch % checkpoint_interval == 0:
            checkpoint = {"model_state_dict": net.state_dict(),
                    "optimizer_state_dict": optimizer.state_dict(),
                    "epoch": epoch+pretrain_epoch}
            path_checkpoint = "./checkpoints/trial1207/all_modify/net__checkpoint_{}_epoch_{}.pkl".format(epoch+pretrain_epoch, test_loss)
            torch.save(checkpoint, path_checkpoint)
            
    checkpoint = {"model_state_dict": net.state_dict(),
                      "optimizer_state_dict": optimizer.state_dict(),
                      "epoch": epochs+pretrain_epoch-1}
    path_checkpoint = "./checkpoints/trial1207/all_modify/net__checkpoint_final_{}_epoch_{}.pkl".format(epochs+pretrain_epoch-1, test_loss)
    torch.save(checkpoint, path_checkpoint)