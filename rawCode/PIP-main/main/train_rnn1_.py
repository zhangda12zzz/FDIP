...
if __name__ == '__main__':
    ...
    # writer = SummaryWriter('runs/trial4/rnn1_PIP')

    val_best_loss = 0.0572
    
    # 预训练的设置
    pretrain_path = 'checkpoints/trial0129/rnn1/best__30_epoch_0.057150669866992576.pkl'
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for batch_idx_val, data_val in enumerate(val_loader):
                ...
                a_loss_val = loss_val.item() / (batch_size * target_val.shape[1])   # 计算平均一帧的mseLoss
        ...
        if (epoch+pretrain_epoch) % checkpoint_interval == 0:
            checkpoint = {"model_state_dict": rnn1.state_dict(),
                    "optimizer_state_dict": optimizer.state_dict(),
                    "epoch": epoch+pretrain_epoch}
            path_checkpoint = "./checkpoints/trial0129/rnn1/checkpoint__{}_epoch_{}.pkl".format(epoch+pretrain_epoch, avg_val_loss)
            torch.save(checkpoint, path_checkpoint)
