...
from GGIP.graph import Graph_B, Graph_J, Graph_P, Graph_A, Unpool
...
class AGGRU_3(nn.Module):   # 输出结果为r6d 15*6
    r'''
        GCN+GRU网络，输入图时序信息，输出预测结果
    '''
...
# class AGGRU_3(nn.Module):   # 输出结果为矩阵15*9
#     r'''
#         GCN+GRU网络，输入图时序信息，输出预测结果
#     '''
...
class GGIP(nn.Module):
    def __init__(self, n_hid_dec=256, strategy='uniform', edge_weighting=True):
        ...
        self.global_to_local_pose = art.ParametricModel(conf.paths.smpl_file).inverse_kinematics_R
        self.loadPretrain()
        self.eval()
    ...
    def forward(self, x, saperateTrain=True):
        ...
        # input3 = torch.concat((p_all, full_acc, full_ori), dim=-1)  #[n,t,24*3+16*12] r6d版本
        input3 = torch.concat((full_acc, full_ori, p_all), dim=-1)  #[n,t,24*3+16*12] 矩阵版本、欧拉角、轴角版本
        if saperateTrain:
