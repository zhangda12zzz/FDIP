...
from model.skeleton import SkeletonUnpool, SkeletonPool, SkeletonConv, SkeletonRnn, find_neighbor, SkeletonLinear
...
class Encoder(nn.Module):
    def __init__(self, args, topology):     # topology是骨骼拓扑、数量比关节少1【v-1】
        ...
        for i in range(args.num_layers):
            ...
            for _ in range(args.extra_conv):
                seq.append(SkeletonRnn(neighbor_list, in_channels=in_channels, out_channels=in_channels,
                                        joint_num=self.edge_num[i], kernel_size=kernel_size, stride=1,
                                        padding=padding, padding_mode=args.padding_mode, bias=bias))
            seq.append(SkeletonRnn(neighbor_list, in_channels=in_channels, out_channels=out_channels,
            ...
            self.layers.append(nn.Sequential(*seq))     # 2层，每层【SkeletonRnn -> SkeletonPool -> LeakyReLU】
    ...
    def forward(self, input, offset=None):  #【Encoder】
        ...
        for i, layer in enumerate(self.layers):
            ...
            input = layer(input)    # 训练时，input:[n,C(4v),t_w(64)] => [n,C'(8v'),t_w] => [n,C''(16v''),t_w]
        return input
...
class Decoder(nn.Module):
    def __init__(self, args, enc: Encoder):
        ...
        for i in range(args.num_layers):
            ...
            self.layers.append(nn.Sequential(*seq)) #2层，每层【SkeletonUnpool->SkeletonRnn->LeakyReLU】
    ...
    def forward(self, input, offset=None):  # Decoder
        for i, layer in enumerate(self.layers):
            ...
            input = layer(input)    # 训练时，input:[n,C''(16v''),t_w] => [n,C'(8v'),t_w] => [n,C(4v), t_w]
...
class AE(nn.Module):
    ...
    def forward(self, input, offset=None):  # input:[n,C(4v-4+3),t_w(64)]
        latent = self.enc(input, offset)    # latent:[n,C''(16v''),t_w]
