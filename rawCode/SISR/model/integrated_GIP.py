...
from model.t_sgrnn import AGGRU
...
class IntegratedModelGIP:
    ...
    def __init__(self, args, num_past_frame=20, num_future_frame=5):
        ...
        if not args.simple_operator:                                        # 包括了3重网络
            ...
            self.pose_encoder = AGGRU(24*3+16*12, 256, 90, strategy='spatial').to(device)
            self.discriminator = MotionDiscriminator(rnn_size=256, input_size=90, num_layers=2, output_size=1).to(device)     # 判别器
        else:
