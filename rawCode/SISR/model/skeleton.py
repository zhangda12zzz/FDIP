...
class SkeletonConv(nn.Module):
    ...
    def forward(self, input):   # Conv
        ...
        # 看一下这里卷积前后的维度： [n,C_in,t] => [n,C_in,7+t+7]（左右各扩充7维）=> [n,C_out, t']，是对时间域的卷积
        # t' = (7+t+7 - kW - 1) / 2 + 1，kW是weight_masked的最后一维，之所以多一个‘-1’是因为step无法整除
        # 最终在训练过程的结果是: 64->32->16(stride=2时), kW和stride都能手动指定、会有不同的输出效果
...
class SkeletonPool(nn.Module):
    ...
    def forward(self, input: torch.Tensor): # Pool
...
class SkeletonRnn(nn.Module):
    def __init__(self, neighbour_list, in_channels, out_channels, kernel_size, joint_num, stride=1, padding=0,
                 bias=True, padding_mode='zeros', add_offset=False, in_offset_channel=0, 
                 rnn_type='gru', num_rnn_layer=2, bidirectional=True, hidden_size=256):
        self.in_channels_per_joint = in_channels // joint_num       # channel_base
        self.out_channels_per_joint = out_channels // joint_num
        if in_channels % joint_num != 0 or out_channels % joint_num != 0:
            raise Exception('BAD')
        super(SkeletonRnn, self).__init__()

        # 不需要padding，所以padding_mode是一个无用数据
        # 也没有卷积操作，所以卷积核相关数据也不需要，但需要计算对应的输入输出尺寸
        ...
        self.rnn = getattr(torch.nn, rnn_type.upper())(in_channels, out_channels, num_rnn_layer,
                                                       bidirectional=bidirectional, batch_first=True)
        self.motion_enc = nn.Linear(out_channels * (2 if bidirectional else 1), out_channels)
        
        # 原本是有个weight的，但是rnn好像不需要这种卷积核，不知道hc需不需要一些好的初始化方法
        # self.reset_parameters()
    ...
    def forward(self, input):    # RNN
        # input:[n,C_in,t]
        frame = input.shape[-1]
        input = input.permute(0,2,1)    #[n,t,C_in]
        # input = input.reshape(input.shape[0], -1)
        # weight_masked = (self.weight * self.mask).to(input.device)
        # bias = self.bias.to(input.device)
        # res = F.linear(input, weight_masked, bias) #[n,t,C_out]
        res = self.rnn(input)[0]
        res = self.motion_enc(res)    #[n,t,c_out]
...
class SkeletonConvRnn(nn.Module):
    def __init__(self, neighbour_list, in_channels, out_channels, kernel_size, joint_num, stride=1, padding=0,
                 bias=True, padding_mode='zeros', add_offset=False, in_offset_channel=0,
                 rnn_type='gru', num_rnn_layer=2, bidirectional=True, hidden_size=256):
        self.in_channels_per_joint = in_channels // joint_num
        ...
        if in_channels % joint_num != 0 or out_channels % joint_num != 0:
            raise Exception('BAD')
        super(SkeletonConvRnn, self).__init__()
        ...
        # self.weight = torch.zeros(out_channels, in_channels, kernel_size)
        self.weight = torch.zeros(out_channels, in_channels, 1)
        if bias:
            self.bias = torch.zeros(out_channels)
        else:
            self.register_parameter('bias', None)
        ...
        self.description = 'SkeletonConvRnn(in_channels_per_armature={}, out_channels_per_armature={}, kernel_size={}, ' \
                           'joint_num={}, stride={}, padding={}, bias={})'.format(
            in_channels // joint_num, out_channels // joint_num, 1, joint_num, 1, padding, bias
        )

        self.reset_parameters()
        self.rnn = getattr(torch.nn, rnn_type.upper())(out_channels, out_channels, num_rnn_layer,
                                                       bidirectional=bidirectional, batch_first=True)
        self.motion_enc = nn.Linear(out_channels * (2 if bidirectional else 1), out_channels)
    ...
    def forward(self, input):   # Conv
        weight_masked = self.weight * self.mask
        res = F.conv1d(input, weight_masked, self.bias, self.stride,
                       0, self.dilation, self.groups) # no padding
        res = res.permute(0,2,1)    #[n,t,C_out]
        res = self.rnn(res)[0]
        res = self.motion_enc(res)    #[n,t,c_out]
        res = res.permute(0,2,1)
...
class SkeletonAttn(nn.Module):
    def __init__(self, neighbour_list, in_channels, out_channels, kernel_size, joint_num, stride=1, padding=0,
                 bias=True, padding_mode='zeros', add_offset=False, in_offset_channel=0,
                 rnn_type='gru', num_rnn_layer=2, bidirectional=True, hidden_size=256, num_heads=4, dropout=0.3, no_mask=True):
        self.in_channels_per_joint = in_channels // joint_num
        self.out_channels_per_joint = out_channels // joint_num
        if in_channels % joint_num != 0 or out_channels % joint_num != 0:
            raise Exception('BAD')
        super(SkeletonAttn, self).__init__()
        ...
        self.linQ = nn.Linear(out_channels, out_channels)
        self.linK = nn.Linear(out_channels, out_channels)
        self.linV = nn.Linear(out_channels, out_channels)
        self.num_heads = num_heads
        self.multihead_attn = nn.MultiheadAttention(out_channels, num_heads=self.num_heads)
        # self.sublayer1 = SublayerConnection(out_channels, dropout)
        self.lin_out = nn.Linear(out_channels, out_channels)
        # self.sublayer2 = SublayerConnection(out_channels, dropout)
        self.laynorm1 = LayerNorm(out_channels)
        self.laynorm2 = LayerNorm(out_channels)
    ...
    def forward(self, input):
        # input:[n,C_in,t]
        input = input.permute(2,0,1)    #[t,n,C_out]
        ...
        mask = None
        if not self.no_mask:
            mask = subsequent_mask(input.shape[0]).to(input.device)
        input = self.multihead_attn(q,k,v,attn_mask=mask)[0] #[t, n, C_out]
        input = input + self.laynorm1(self.dropout(input))
        res = input + self.laynorm2(self.lin_out(input))     # [t,n,C_out]
...
def build_edge_topology(topology, offset):
    ...
    return edges    # [v-1, 3]，每个包括了【父节点、自节点、offset】
...
def find_neighbor(edges, d):
    ...
    for i in range(edge_num):
        neighbor = []
        for j in range(edge_num):
            if edge_mat[i][j] <= d: # 将相距的关节数不超过 d 的骨骼视为相邻骨骼
                neighbor.append(j)
        neighbor_list.append(neighbor)
