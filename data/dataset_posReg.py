...
class ImuDataset(Dataset):
    def __init__(self, data_folders):
        ...
        for data_folder in data_folders:
            self.pose_raw += torch.load(
                data_folder + '/pose.pt')  # self.data_dict['pose']  # list[ tensor[[?, 24, 3, 3]] ]          smpl姿态参数，24个节点的旋转
            self.acc_raw += torch.load(
                data_folder + '/vacc.pt')  # self.data_dict['acc']    # list[ tensor[[?, 6, 3]] ]           6个节点的加速度             已经标准化
            self.ori_raw += torch.load(
                data_folder + '/vrot.pt')  # self.data_dict['ori']    # list[ tensor[[?, 6, 3, 3]] ]           6个节点的方向               已经标准化
            ...
            self.tran_raw += torch.load(
                data_folder + '/tran.pt')  # self.data_dict['tran']          # list[ tensor[[?, 3]] ]     根节点位置
            self.joint_raw += torch.load(
                data_folder + '/joint.pt')  # self.data_dict['joint']        # list[ tensor[[?, 24, 3]] ]   24个关节的绝对位置（预处理已经加上了根节点）
            # self.vel_raw += torch.load(data_folder + '/jvel.pt')    #关节速度信息

            self.beta_raw += torch.load(
                data_folder + '/shape.pt')  # self.data_dict['shape']        # list[ tensor[10] ]    smpl形状参数
    ...
    def __getitem__(self, index):
        ...
        p_all_relative = torch.cat((torch.zeros_like(joint[:, 0:1]), joint[:, 1:] - joint[:, 0:1]), dim=1)
        out_all_pos_xiangdui = p_all_relative[:, conf.joint_set.full]
        ...
        out_tran = self.tran[index]

        # return out_acc, out_ori, out_leaf_pos, out_all_pos, out_grot, grot_reduce, out_grot_axis, out_grot_euler, out_grot_quat
        return out_acc, out_ori, out_rot_6d, out_leaf_pos, out_all_pos, out_pose, out_pose_6d, out_shape, out_tran, out_all_pos_xiangdui
...
if __name__ == '__main__':
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        out_all_pos = data[8].to(device).float()
