    ...
    def __init__(self, input_channels, base_channels, dilation_rates):
        ...
        for dilation in dilation_rates:
            branch = nn.Sequential(
                nn.Conv1d(input_channels, base_channels // 2, kernel_size=3, dilation=dilation, padding=dilation),
...
class NodeAwareMSFKE(nn.Module):
    def __init__(self,
        ...
        if self.num_nodes == 6:
            ...
            self.node_to_group_map = {0: 0, 3: 1, 1: 2, 2: 2, 4: 2, 5: 2}  # 6节点的分组
            ...
            self.dimension_adapter = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Dropout(0.1),
                nn.LayerNorm(self.num_nodes * self.adapted_input_channels)
            )
        ...
        # --- 2. 定义模型组件 ---
        # [关键修正] node_embedding 参数尺寸固定为最大可能节点数
        ...
        self.shared_projector = nn.Conv1d(
            self.fused_channels + self.node_embedding_dim,
    ...
    def forward(self, imu_data):
        ...
        # [关键修正] 在前向计算中，根据 self.num_nodes 切片出需要的 embedding
