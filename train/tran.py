...
def train_fdip_1(train_loader, val_loader, pretrained_epoch=0, epochs=1):
    ...
    # 初始化 tqdm 总进度条
    overall_pbar = tqdm(total=epochs, desc="FDIP_1 Training Progress", position=0)
    ...
    for epoch in range(epochs):
        ...
        # 验证进度条
        val_pbar = tqdm(total=len(val_loader), desc="Validation Progress", position=1, leave=False)
        ...
        with torch.no_grad():
            for data_val in val_loader:
                try:
                    ...
                    # 更新验证进度条
                    val_pbar.update(1)
                    val_pbar.set_postfix(val_loss=loss_val.item())
                except Exception as e:
    ...
    train_pbar = tqdm(total=len(train_loader), desc="Generating Train Predictions", position=1)
    val_pbar = tqdm(total=len(val_loader), desc="Generating Val Predictions", position=2)

    train_predictions = predict_fdip_1(model, train_loader, train_pbar)
    val_predictions = predict_fdip_1(model, val_loader, val_pbar)
...
def predict_fdip_1(model, data_loader, progress_bar=None):
...
def train_fdip_2(train_loader, val_loader, train_predictions, val_predictions, pretrained_epoch=0, epochs=1):
    ...
    # 初始化 tqdm 总进度条
    overall_pbar = tqdm(total=epochs, desc="FDIP_2 Training Progress", position=0)
    ...
    train_predictions_2 = predict_fdip_2(model, train_loader, train_predictions, train_pbar)
    val_predictions_2 = predict_fdip_2(model, val_loader, val_predictions, val_pbar)
...
def predict_fdip_2(model, data_loader, aggru1_predictions, progress_bar=None):
...
def train_fdip_3(train_loader, val_loader, train_predictions_2, val_predictions_2, pretrained_epoch=0, epochs=1):
    ...
    # 初始化 tqdm 总进度条
    overall_pbar = tqdm(total=epochs, desc="FDIP_3 Training Progress", position=0)
    ...
    for epoch in range(epochs):
        ...
        if LOG_ENABLED and writer:
            # 记录软约束层的最小角度和最大角度
            writer.add_histogram(f'limits/min_angles_epoch_{current_epoch}', model.soft_limits.min_angles.data,
                                 current_epoch)
            writer.add_histogram(f'limits/max_angles_epoch_{current_epoch}', model.soft_limits.max_angles.data,
                                 current_epoch)

            # 打印当前角度限制
            print(
                f"Epoch {current_epoch} - Min angles: {model.soft_limits.min_angles.data.min().item():.4f} to {model.soft_limits.min_angles.data.max().item():.4f}")
            print(
                f"Epoch {current_epoch} - Max angles: {model.soft_limits.max_angles.data.min().item():.4f} to {model.soft_limits.max_angles.data.max().item():.4f}")
...
def evaluate_pipeline(model1, model2, model3, data_loader):
    ...
    # 评估进度条
    eval_pbar = tqdm(total=len(data_loader), desc="Evaluating Pipeline", position=0)
...
def main():
    ...
    # 主训练进度条
    main_pbar = tqdm(total=5, desc="Overall Training Progress", position=0)
    ...
    main_pbar.update(1)
    main_pbar.set_description("Training FDIP_1 completed")
    ...
    main_pbar.update(1)
    main_pbar.set_description("Training FDIP_2 completed")
    ...
    main_pbar.update(1)
    main_pbar.set_description("Training FDIP_3 completed")
    ...
    # 评估阶段
    _, _ = evaluate_pipeline(model1, model2, model3, val_loader)
    main_pbar.update(1)
    main_pbar.set_description("Evaluation completed")

    main_pbar.update(1)
    main_pbar.set_description("All tasks completed")
    main_pbar.close()
